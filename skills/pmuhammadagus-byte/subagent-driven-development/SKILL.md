...
description: "Gunakan saat ada rencana implementasi dengan tugas yang sebagian besar independen dan ingin dieksekusi via subagent paralel di sesi yang sama (bukan context switch). Aktif saat user minta "jalankan plan pakai subagent", "dispatch implementer per task", atau "subagent-driven dev"." [shape=box];; plans" [shape=box];. Aktif untuk tugas terkait skill ini."
metadata:
  openclaw:
    version: 1.0.0
---
...
# subagent-driven-development — X∞ Compliance Layer

## 1. IDENTITY
Skill milik user: `subagent-driven-development`. Mengikuti Skill Architecture Standard X∞ (wajib).

## 2. PURPOSE
Mengeksekusi rencana dengan mendispatch subagent implementer baru (fresh) per tugas, lalu review per-tugas (spec compliance + kualitas kode) dan review menyeluruh di akhir branch—iterasi cepat tanpa context pollution.
