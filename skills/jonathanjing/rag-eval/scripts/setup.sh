...
if [[ -z "${VIRTUAL_ENV:-}" ]]; then
    err "No virtual environment detected; refusing to modify system Python."
...
if [[ -z "${OPENAI_API_KEY:-}" && -z "${ANTHROPIC_API_KEY:-}" && -z "${RAGAS_LLM:-}" ]]; then
    err "Configure a local RAGAS_LLM or an approved cloud judge before evaluation."
    echo "  Example local judge: export RAGAS_LLM=ollama/llama3"
    echo ""
fi
...
echo ""
echo "Quick test:"
echo "  Write the evaluation JSON to /tmp/rag-eval-input.json, then run:"
echo "  python3 scripts/run_eval.py --input-file /tmp/rag-eval-input.json"
echo ""
