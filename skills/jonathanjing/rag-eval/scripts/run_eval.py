...
Flags:
    ...
    --save                  Persist the raw input and result to memory/eval-results/
"""
...
def _get_llm():
    ...
    Priority:
      1. RAGAS_LLM=ollama/<model> → local Ollama (offline)
      2. ANTHROPIC_API_KEY        → Claude Haiku (fast, cheap)
      3. OPENAI_API_KEY           → GPT-4o-mini
    """
    ragas_llm_env = os.environ.get("RAGAS_LLM", "")
    allow_cloud = os.environ.get("RAGAS_ALLOW_CLOUD") == "1"

    # An explicit local judge always wins over ambient cloud credentials.
    if ragas_llm_env.startswith("ollama/"):
        try:
            from langchain_community.chat_models import ChatOllama
            from ragas.llms import LangchainLLMWrapper
            model_name = ragas_llm_env.split("/", 1)[1].strip()
            if not model_name:
                _die(
                    "Ollama model name is empty.",
                    fix="Set RAGAS_LLM=ollama/<model>, for example ollama/llama3.",
                )
    ...
    if ragas_llm_env:
        _die(
            f"Unsupported RAGAS_LLM value: {ragas_llm_env}",
            fix="Use RAGAS_LLM=ollama/<model> or unset it to select an approved cloud judge.",
        )

    # Anthropic — preferred when key present.
    if os.environ.get("ANTHROPIC_API_KEY"):
        if not allow_cloud:
            _die(
                "Cloud judge use requires explicit approval.",
                fix="Set RAGAS_ALLOW_CLOUD=1 only after approving transmission of question, answer, and contexts.",
            )
        try:
    ...
    _die(
        "No LLM configured. Set a local RAGAS_LLM or an approved cloud judge.",
        fix=(
...
def _get_embeddings():
    ...
    # OpenAI embeddings are cloud processing and require the same explicit
    # egress approval as the judge. An explicit local judge also forces local
    # embeddings even when cloud credentials are present.
    if (
        os.environ.get("OPENAI_API_KEY")
        and os.environ.get("RAGAS_ALLOW_CLOUD") == "1"
        and not ragas_llm_env.startswith("ollama/")
    ):
        try:
...
def main() -> None:
    ...
    parser.add_argument(
        "--save",
        action="store_true",
        help="Persist the raw input and result to memory/eval-results/YYYY-MM-DD.jsonl",
    )
    parser.add_argument(
        "--input-file",
        type=str,
