...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(default: ./batch-eval-YYYY-MM-DD.json)"
        ),
    )
    parser.add_argument(
        "--metric",
    ...
    parser.add_argument(
        "--include-content",
        action="store_true",
        help="Include raw questions and references in the JSON report",
    )
    parser.add_argument(
        "--save-individual",
        action="store_true",
        help="Persist each raw question, answer, and context record to memory/eval-results/",
    )
    ...
    if args.output:
        ...
        out_path = Path.cwd() / f"batch-eval-{today}.json"
    ...
    for i, record in enumerate(records, 1):
        ...
        try:
            ...
            if args.include_content:
                result_entry["question"] = q
            if ref and args.include_content:
